"""Сброс демо-данных (зоны, ПК, тарифы, товары, промо, настройки, сотрудник).

Требуются таблицы из backend/schema.sql (MySQL) или совместимая схема.
"""
...
class Command(BaseCommand):
    help = "Очистить ключевые таблицы и загрузить демо-данные."
        ...
        with connection.cursor() as c:
            ...
            for table in (
                ...
                try:
                    ...
                    err = str(e)
                    if "doesn't exist" not in err and "1146" not in err:
            ...
            c.execute(
                """
                INSERT INTO clients (name, phone, balance, total_sessions) VALUES
                ('Алексей «Frag»', '+79001234567', 1500.00, 12),
                ('Мария К.', '+79007654321', 350.50, 4),
                ('Тимур', '+79005551122', 0.00, 0)
                """
            )
            demo_pin = make_password("1234")
            c.execute(
                "UPDATE clients SET player_pin_hash = %s WHERE player_pin_hash IS NULL",
                [demo_pin],
            )

            self.stdout.write(
                self.style.SUCCESS(
                    "Готово: 40 ПК, 5 тарифов, 6 товаров, 3 промокода, настройки, зоны, сотрудник. Логин: admin / admin"
                )
            )
