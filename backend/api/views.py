...
def _insert_transaction(
    ...
    try:
        cursor.execute(
            ...
            (client_id, amount, type, method, description, employee_id, shift_id)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
            """,
            [client_id, amount, typ, method, description or "", employee_id, sid],
        )
    except Exception:
        cursor.execute(
            """
...
def sessions(request):
    ...
    if request.method == "POST":
        ...
        if dm is None and tariff_id:
            with connection.cursor() as c:
                c.execute(
                    "SELECT duration_minutes FROM tariffs WHERE id = %s", [tariff_id]
                )
                tr = dictfetchone(c)
                dm = tr["duration_minutes"] if tr else 0
        try:
            dm = int(dm or 0)
        except (TypeError, ValueError):
            ...
            with transaction.atomic():
                with connection.cursor() as c:
                    c.execute(
                        """
                        INSERT INTO sales
                        (shift_id, client_id, total, payment_method, cash_given, change_given)
                        VALUES (%s,%s,%s,%s,%s,%s)
                        """,
                        [
                            shift_id,
...
def promos_apply(request):
    ...
    if not promos:
        return json_response({"error": "Промокод не найден или неактивен"}, 404)
    promo = promos[0]
    if promo["used_count"] >= promo["max_usage"]:
        return json_response({"error": "Лимит использований исчерпан"}, 400)
    if promo["expires_at"]:
        exp = promo["expires_at"]
        if hasattr(exp, "date"):
            exp = exp.date()
        if exp < date.today():
            return json_response({"error": "Промокод истёк"}, 400)
    bonus_added = 0
    with connection.cursor() as c:
        c.execute(
